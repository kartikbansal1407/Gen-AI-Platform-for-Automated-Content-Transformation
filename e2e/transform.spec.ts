...
test("authenticated workspace: PDF, three outputs, refinement, review, documents, analytics and deletion", async ({
  page,
}) => {
  await login(page);
  ...
  await expect(
    page.getByRole("checkbox", { name: /Executive Summary/ }),
  ).toBeChecked();
  ...
  ).toContainText("18 advisories");
  await page.getByRole("checkbox", { name: /Advisory/ }).click();
  await page
    .getByText("Additional publishing formats", { exact: true })
    .click();
  ...
  await page
    ...
    .click();
  await expect(
    page.getByRole("heading", { name: "3 artefacts · done" }),
  ).toBeVisible();
  await expect(page.getByRole("tab")).toHaveCount(3);
  await page.getByRole("tab", { name: "Advisory", exact: true }).click();
  await expect(
    page.getByRole("article", { name: "advisory artefact" }),
  ).toContainText("18 advisories");
  ...
  await page
    .getByRole("button", { name: "incident-report.pdf", exact: true })
    .click();
  await expect(page.locator("pre")).toContainText("18 advisories");
  await page
    .getByRole("link", { name: "Open transformation", exact: true })
    .click();
  await expect(page.getByRole("tab")).toHaveCount(3);
  await page.getByRole("link", { name: "Review", exact: true }).click();
  await page
    .getByRole("button")
    .filter({ hasText: "Advisory · pending" })
    .click();
  ...
  await page
    .getByRole("button")
    .filter({ hasText: "Advisory · approved" })
    .click();
  ...
  await page.getByRole("tab", { name: "Advisory", exact: true }).click();
  await page
    .getByLabel("Refine this artefact")
    .fill("Make this advisory more formal.");
  await page.getByRole("button", { name: "Refine", exact: true }).click();
  await expect(page.getByRole("status")).toContainText("Artefact updated.");
  await navigate(page, "Review Queue");
  await expect(
    page.getByRole("button").filter({ hasText: "Advisory · pending" }),
  ).toBeVisible();
  await navigate(page, "Analytics");
  await expect(
    page.getByText("Based on 1 loaded transformations.", { exact: false }),
  ).toBeVisible();
  await expect(
    page.getByText("Generated output formats", { exact: true }),
  ).toBeVisible();
  await expect(page.getByText(/engagement|followers|impressions/i)).toHaveCount(
    0,
  );
  await navigate(page, "Settings");
  await expect(
    page.getByText(
      /platform integrations|personal memory|LinkedIn|Reddit|campaigns/i,
    ),
  ).toHaveCount(0);
  ...
  expect((await workspaceDownload).suggestedFilename()).toBe(
...
test("rejects wrong access codes and supports mobile navigation", async ({
  page,
}) => {
