...
export function TransformDashboard() {
  ...
  async function handleGenerate() {
    ...
    try {
      const res = await fetch("/api/transform", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ text, url: url || undefined, controls }) });
      const data = await res.json();
      if (!res.ok) throw new Error(data.error ? JSON.stringify(data.error) : "Failed");
      setArtefacts(data.artefacts);
      // cache to localStorage for demo
  ...
  return (
    <div className="space-y-6">
      ...
      <p className="text-sm opacity-70">Source → Controls → Outputs (SIH PS 26154)</p>
      <IngestDropzone text={text} setText={setText} url={url} setUrl={setUrl} />
      ...
      <button onClick={handleGenerate} disabled={loading || controls.outputs.length===0} className="h-11 rounded-md bg-[#111] px-6 text-white disabled:opacity-40">{loading ? "Generating…" : "Generate"}</button>
