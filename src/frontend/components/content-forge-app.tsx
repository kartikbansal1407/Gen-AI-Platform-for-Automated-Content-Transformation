...
export function ContentForgeApp({ section }: { section: ProductSection }) {
  ...
  return (
    <div className={`${theme === "dark" ? "dark" : ""} min-h-screen bg-[var(--background)] text-[var(--foreground)]`}>
      <aside
        className={`${menu ? "visible translate-x-0" : "invisible -translate-x-full lg:visible"} fixed inset-y-0 left-0 z-50 flex w-[280px] flex-col border-r border-[var(--line)] bg-[var(--surface)] px-4 py-5 transition-[transform,visibility] duration-200 lg:translate-x-0`}
      >
        ...
        <Link href="/transform" onClick={() => setMenu(false)} className="mb-5 flex items-center justify-between rounded-2xl bg-[#173f4f] px-4 py-3.5 text-sm font-semibold text-white shadow-sm transition hover:bg-[#0f303d] dark:bg-[#d9ff72] dark:text-[#10232b] dark:hover:bg-[#efffb9]">
        ...
        <div className="mt-auto rounded-2xl border border-[var(--line)] bg-[var(--background)] p-3.5">
          <div className="mb-3 flex items-center gap-2 text-xs font-medium"><span className="size-2 rounded-full bg-emerald-500" />Operator workspace</div>
          <p className="mb-3 text-[11px] leading-5 text-[var(--muted)]">Source-faithful generation. Human approval stays in the loop.</p>
          <button onClick={signOut} className="flex w-full items-center gap-2 rounded-xl px-2 py-2 text-left text-xs font-medium text-[var(--muted)] hover:bg-[var(--surface-muted)] hover:text-[var(--foreground)]"><LogOut aria-hidden className="size-4" />Sign out</button>
        </div>
      </aside>
      ...
      <div className="lg:pl-[280px]">
        <header className="sticky top-0 z-30 flex h-[72px] items-center justify-between border-b border-[var(--line)] bg-[color:var(--background)]/90 px-5 backdrop-blur-xl lg:px-8">
          <div className="flex min-w-0 items-center gap-3">
            <button aria-label="Open navigation" className="rounded-xl border border-[var(--line)] bg-[var(--surface)] p-2 lg:hidden" onClick={() => setMenu(true)}><Menu className="size-5" /></button>
            <div className="min-w-0"><p className="truncate text-sm font-semibold">{current?.label}</p><p className="hidden truncate text-xs text-[var(--muted)] sm:block">{current?.description}</p></div>
          </div>
        ...
        <main className="mx-auto max-w-[1480px] p-4 sm:p-6 lg:p-8">
          {(sessionError || data.error) && <div role="alert" className="mb-5 rounded-2xl border border-amber-300/70 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-100">{sessionError || data.error}</div>}
          ...
          {section === "transform" ? (
            ...
            <div className="space-y-6">
              <section className="flex flex-col gap-3 border-b border-[var(--line)] pb-6 sm:flex-row sm:items-end sm:justify-between">
                ...
                {section !== "dashboard" && section !== "settings" && <Link href="/transform" className="inline-flex items-center justify-center gap-2 rounded-xl bg-[#173f4f] px-4 py-2.5 text-sm font-semibold text-white hover:bg-[#0f303d] dark:bg-[#d9ff72] dark:text-[#173f4f]"><Plus className="size-4" />New Transformation</Link>}
              </section>

              {data.loading ? (
                <div role="status" className="grid min-h-[280px] place-items-center rounded-3xl border border-dashed border-[var(--line)] bg-[var(--surface)]"><div className="text-center"><Orbit className="mx-auto mb-3 size-6 animate-spin text-[var(--muted)]" /><p className="text-sm text-[var(--muted)]">Loading workspace…</p></div></div>
              ) : (
                <>
                  {section === "dashboard" && <DashboardView jobs={data.jobs} />}
                  {section === "documents" && <DocumentsView jobs={data.jobs} />}
                  {(section === "workspace" || section === "history") && <WorkspaceView key={section} jobs={data.jobs} history={section === "history"} onUpdate={data.update} onRemove={data.remove} />}
